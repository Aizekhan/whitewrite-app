...
- **P2** — продакшн (in-browser Babel, dev-React, 3 МБ PNG); memo для `wConnections`/`wImpact` (O(N²)); мобільні overlay/граф; персистентність (усе в памʼяті, крім image-slots).
